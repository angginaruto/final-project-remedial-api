...
export const getShiftReportService = async (userId: number, date?: string) => {
  let dateFilter = {};
...
export const getDailySalesReportService = async (
  ...
  const where: any = { cashier: { createdById: userId } };
  ...
  const dailyMap = new Map<
    string,
    { totalTransactions: number; totalSales: number; totalCash: number; totalDebit: number }
  >();
...
export const getDailyProductSalesReportService = async (
  userId: number,
  startDate?: string,
  endDate?: string
) => {
  const where: any = { transaction: { cashier: { createdById: userId } } };
  ...
  const dailyMap = new Map<
    string,
    { productId: number; productName: string; quantitySold: number; totalSales: number }
  >();
...
export const getDiscrepancyReportService = async (userId: number) => {
